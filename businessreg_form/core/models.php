...
function insertNewUser($pdo, $first_name, $last_name, $email, $phone_number, $username, $hashedPassword) {
    $checkUserSql = "SELECT * FROM user_passwords WHERE username = ?";
    $checkUserSqlStmt = $pdo->prepare($checkUserSql);
    $checkUserSqlStmt->execute([$username]);

    if ($checkUserSqlStmt->rowCount() == 0) {
        $sql = "INSERT INTO user_passwords (first_name, last_name, email, phone_number, username, password) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $pdo->prepare($sql);
        $executeQuery = $stmt->execute([$first_name, $last_name, $email, $phone_number, $username, $hashedPassword]);

        if ($executeQuery) {
            $_SESSION['message'] = "User successfully registered!";
            return true;
        } else {
            $_SESSION['message'] = "An error occurred while registering the user.";
        }
    } else {
...
function insertBarista($pdo, $username, $first_name, $last_name, $added_by) {
    $sql = "INSERT INTO barista (username, first_name, last_name, added_by) VALUES(?, ?, ?, ?)";
    $stmt = $pdo->prepare($sql);
    $executeQuery = $stmt->execute([$username, $first_name, $last_name, $added_by]);
...
function updateBarista($pdo, $first_name, $last_name, $barista_id) {
    $sql = "UPDATE barista SET first_name = ?, last_name = ?, 
    last_updated = NOW() WHERE barista_id = ?";
    $stmt = $pdo->prepare($sql);
